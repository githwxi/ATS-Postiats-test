...
extern
fun
nth_perm(n: int, xs: list0(int)): list0(int)
...
implement
main0(argc, argv) = let
  val input =  (if (argc >= 2) then g0string2int_int(argv[1]) else 9): int
  val xs = make_list0(input)
  val () = println!("Your list = ")
  val () = print_lst(xs)
  val () = println!()
  val () = println!("1 000 000th Permutation = ")
  val () = print_lst(nth_perm(1000000, xs))
in
  println!()
end
