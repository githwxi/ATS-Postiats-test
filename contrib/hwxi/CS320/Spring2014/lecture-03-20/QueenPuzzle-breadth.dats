(* ****** ****** *)
//
// HX: for a lecture on breadth-first search
//
(* ****** ****** *)
...
datatype
qnode = QNX of (int, int)

(* ****** ****** *)

assume $TS.node_type = list0(qnode)
...
$TS.node_get_children
  (nx0) = let
//
val-cons0(qx, _) = nx0; val QNX(i, _) = qx
//
in
  list0_tabulate<node> (N, lam n => cons0 (QNX (i+1, n), nx0))
...
val+QNX(i0, j0) = qx
//
fun test (qxs: list0(qnode)): bool =
  case+ qxs of
  | nil0 () => true
  | cons0 (QNX(i, j), qxs) =>
      if (j0 != j) && abs(i0-i) != abs(j0-j) then test (qxs) else false
    // end of [cons0]
//
val ans = test (qxs)
//
...
(
  ...
  case+ qxs of
  | nil0 () => ()
  | cons0 (QNX(i, j), qxs) => (the_board[i, j] := 1; loop (qxs))
//
val () = loop (path)
//
...
val nxs0 =
list0_tabulate<node>
  (N, lam n => list0_sing(QNX(0, n)))
//
val ((*void*)) = $BF.breadth_search_list (nxs0)
//
...
gtkcairotimer_title<>
(
) = stropt_some"QueenPuzzle-breadth"
implement
...
(* end of [QueenPuzzle-breadth.dats] *)
